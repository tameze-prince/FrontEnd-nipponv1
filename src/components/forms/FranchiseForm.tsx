...
export default function FranchiseForm({
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim()) {
      nextErrors.name = 'Le nom de la franchise est requis.';
    }
  ...
  const handleLogoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (loadEvent) => setPreviewLogo(loadEvent.target?.result as string);
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-2xl space-y-6">
      ...
      <div>
        ...
        {previewLogo && (
          <div className="mt-4">
            ...
            <img src={previewLogo} alt="Logo preview" className="h-32 w-32 rounded object-cover" />
          </div>
        )}
      </div>
      ...
      >
        ...
        {loading ? 'Enregistrement...' : isEdit ? 'Mettre a jour la franchise' : 'Creer la franchise'}
      </Button>
    </form>
  );
}
